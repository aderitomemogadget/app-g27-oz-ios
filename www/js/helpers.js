...
function show_erro(MSG,DURATION)
{
	if(window.plugins)
	{
...
function registo1_step1()
{
	...
	$.ajax({
		...
		success:function(data)
		{
			...
			{
				//erro
				$('#registo-1').html('<div style="display:block;background-color:#fff"><div class="header"><div style="padding:10px;width:50%;float:left"><img src="img/logo.svg" border="0" style="width:80px;float:left"></div><div style="padding-right:10px;width:50%;float:left;text-align:right"><img src="img/hello.png" border="0" style="width:80px"></div></div></div><div style="height:20px;background:#fff;">&nbsp;</div><div class="mensagens-container" style="margin-top:-20px;background:#fff;padding-top:20px"><div id="lista-scroll" class="lista-scroll" style="overflow:hidden;overflow-y:scroll"> ERRO <div class="clear"></div><div style="display:block;width:100%;text-align:center;margin-top:20px"><div class="button_reserva cancelar-btn-trad" onclick="goto_to_start()">Voltar</div></div></div></div></div>');
			}
		},
		error:function()
		{
...
function registo2_step1()
{
	...
	$.ajax({
		...
		success:function(data)
		{
			...
			{
				//erro
				$('#registo-2').html('<div style="display:block;background-color:#fff"><div class="header"><div style="padding:10px;width:50%;float:left"><img src="img/logo.svg" border="0" style="width:80px;float:left"></div><div style="padding-right:10px;width:50%;float:left;text-align:right"><img src="img/hello.png" border="0" style="width:80px"></div></div></div><div style="height:20px;background:#fff;">&nbsp;</div><div class="mensagens-container" style="margin-top:-20px;background:#fff;padding-top:20px"><div id="lista-scroll" class="lista-scroll" style="overflow:hidden;overflow-y:scroll"> ERRO <div class="clear"></div><div style="display:block;width:100%;text-align:center;margin-top:20px"><div class="button_reserva cancelar-btn-trad" onclick="goto_to_start()">Voltar</div></div></div></div></div>');
			}
		},
		error:function()
		{
...
function registo1_step2()
{
	...
	if(condutor_nome == '' || condutor_email == '' || telemovel == '' || morada == '' || codpos1 == '' || codpos2 == '' || matricula == '' || localidade_txt=='' || nif=='')
	{
		...
		$.ajax({
			...
			success:function(data)
			{
				if(data != 0 || data != '')
				{
					...
					html = '<div class="app-header"><div class="header"><div class="left"><img src="img/logo.svg" alt="" border="0" class="logo"></div><div class="cent"></div><div class="right"><a class="profile-toggle" href="#profile-panel"><img class="close" src="img/close2.svg"></a></div></div></div>';
...
function getListaPontosAbastecimento(e)
{
	...
	$.ajax({
		...
		async:true,
		timeout: 30000,
		data:{'type':'getListaPontosAbastecimento','dp':(new Date()).getTime(),'token':window.localStorage.getItem("token"),'lat':window.localStorage.getItem("lat"),'lng':window.localStorage.getItem("lng")},
		dataType:'html',
		success:function(data)
		{
			html += data;
		...
		error:function()
		{
			show_erro("Não foi possivel realizar o pedido!",2000);
			window.location.hash='homepage';
		}
	});
}
...
function showSliderPostos(e,vpw)
{
	...
	$.ajax({
		...
		async:true,
		timeout: 30000,
		data:{'type':'showSliderPostos','dp':(new Date()).getTime(),'pagar_loja':window.localStorage.getItem("pagar_loja"),'telemovel' :window.localStorage.getItem("telemovel"),'token':window.localStorage.getItem("token"),'lat':window.localStorage.getItem("lat"),'lng':window.localStorage.getItem("lng")},
		dataType:'html',
		success:function(data)
		{
			html = '<div id="show-all-postos" style="display:inline-block;width:150px;float:left"><div class="ver-mapa" style="margin-left:20px" onclick="window.location.hash=\'location\'">Ver lista de postos</div></div>';
			html += '<div style="display:inline-block;width:35px;float:right"><img onclick="google_api_position();initMapAppHomepage(window.localStorage.getItem(\'raio_pesquisa\'));actualizaSliderPostos()" src="img/icon_posto_mira.svg" style="width:25px"></div>';
...
function carrega_saldo_mbway()
{
	...
	{
		show_erro('Aguarde por favor',2000);		
		$.ajax({
			type:'POST',
			timeout:10000,
			...
			success:function(data)
			{
				if(data == 1)
				{
					show_erro('Carregamento MBWAY processado com sucesso. Faça o pagamento na sua APP MBWAY.',2000);					
					setTimeout(function()
					{
						window.location.hash = 'specific-location';
					},15000);
				...
				{
					show_erro('Não foi possível processar o pagamento MBWAY.',2000);					
				}
				// html += data;
			},
			error:function(data)
	...
	{
		show_erro('Introduza um valor superior a 0.',2000);		
	}
	else if(descricao_mbway == '')
	{
		show_erro('Introduza uma descrição relativamente ao carregamento MBWAY.',2000);				
	}
}
	...
	if(valor_mbway > 0 && descricao_mbway != '')
	{
		...
		$.ajax({
			...
			success:function(data)
			{
				if(data == 1)
				{
					show_erro('Deverá receber em breve o pedido de pagamento na sua aplicação MB WAY. Caso não receba nenhuma notificação, verifique o pagamento dentro da aplicação, no separador ATIVIDADE',2000);					
					$('#valor').val('0.00');
...
function cancelarPaymentConfirmation_2()
{
	...
	$.ajax({
		...
		success:function(data)
		{
			show_erro('Pagamento Cancelado.',2000);			
		...
		error:function(data)
		{
			...
			{
				show_erro('Erro ao tentar cancelar o pagamento.',2000);				
			}
		}
	});
}
...
function confirmPaymentValido()
{
	...
	$.ajax({
		...
		success:function(data)
		{
			...
			if(estado == 1)
			{
				show_erro('O pagamento foi efectuado com sucesso.',2000);				
				timer = setTimeout(function()
				{
					window.location.hash = 'homepage';
				},8000);
			...
			else
			{
				if(estado == 0)
				{
					show_erro('Não foi possível fazer o pagamento, tente novamente mais tarde',2000);
				}
				if(estado == 2)
				{
					show_erro('Não foi possível fazer o pagamento, contem artigos não contratualizados',2000);
				}
				if(estado == 3)
				{
					show_erro('Não foi possível fazer o pagamento, tente novamente mais tarde',2000);
				...
				if(estado == 6)
				{
					show_erro('Não foi possível fazer o pagamento Excedeu o seu limite diário.',2000);
				}
				if(estado == 7)
				{
					show_erro('Não foi possível fazer o pagamento, Cliente Bloqueado Contacte HiQi.',2000);
				}
				if(estado == 8)
				{
					show_erro('Não foi possível fazer o pagamento, estes produtos não estão contratualizados.',2000);
				}
		...
		error:function(data)
		{
			show_erro('Erro ao aceder ao serviço, tente novamente...',2000);
		}
	});
...
function confirmPayment_2()
{
	...
	$.ajax({
		...
		success:function(data)
		{
			...
			{
				show_erro('O pagamento foi cancelado.',2000);
			...
			{
				show_erro('Não existem pagamentos.',2000);
			...
			{
				show_erro('Contem Produtos não contratualizados,não sendo possível realizar o pagamento.',2000);				
			...
			{
				show_erro('Litros Abastecidos superior ao limite permitido.',2000);
			...
			{
				show_erro('Cliente Bloqueado.',2000);
		...
		error:function(data)
		{
			...
			{
				show_erro('Erro, tente novamente.',2000);
			}
		}
	});
}
function initiatePayment()
{
	...
	setTimeout(function(){initiatePayment_2();},1000);
	...
	$.ajax({
		...
		success:function(data)
		{
			...
			else
			{
				...
				show_erro('Não foram detectados pagamentos para aprovação, tente novamente',2000);
			}
		},
		error:function(data)
		{
...
{
	var posto_info='POSTO HiQi';
	var posto_name=window.localStorage.getItem("postoPOS");
	header = '<div class="app-header"><div class="header"><div class="left"><img src="img/logo.svg" alt="" border="0" class="logo"></div><div class="cent"></div></div></div>';
	$("#header").html(header);
...
{
	var posto_info='CALLCENTER HiQi';
...
function valida_token()
{
	...
	$.ajax({
			...
			success:function(data)
			{
				...
				{
					show_erro('Token Incorrecto.',2000);					
...
function validaTelemovelAlterarMatricula()
{
	...
	{
		show_erro('Necessário preencher o número de telemóvel.',2000);
	}
	else
	{
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				{
					show_erro('O número de telemóvel não é válido.',2000);
				}
			},
			error:function(data)
			{
...
function alterarMatricula()
{
	...
	$.ajax({
		...
		success:function(data)
		{
			...
			if(STATUS == 'SUCESS')
			{
				show_erro('Matrícula alterada com sucesso.',2000);
...
function criarMatricula()
{
	...
	$.ajax({
				...
				success:function(data)
				{
					...
					if(STATUS == 'SUCESS')
					{
						show_erro('Matrícula criada com sucesso.',2000);
						window.localStorage.setItem("backup_matricula_pos",window.localStorage.getItem("matricula"));
						setTimeout(function()
						{
							window.location.hash = 'iniciar-pagamento-callcenter';
...
{
	var limite_diario = parseFloat(window.localStorage.getItem("limite_diario"));
	var limite_semanal = parseFloat(window.localStorage.getItem("limite_semanal"));
	var limite_mensal = parseFloat(window.localStorage.getItem("limite_mensal"));
	...
	if(limite_diario > 0 && limite_semanal > 0 && limite_mensal > 0 && nome != '' && (($('#pin').val().length > 3 && $('#pin').val().length < 7) || pin =='' || pin == '0000'))
	{
		...
		$.ajax({
				...
				success:function(data)
				{
					...
					if(STATUS == 'SUCESS')
					{
						show_erro('Cartão alterado com sucesso.',2000);
					...
					{
						show_erro('Não foi possível alterar o Cartão.',2000);
					}
				},
				error:function(data)
				{
	...
	else
	{
		if(limite_diario <= 0)
		{
			show_erro('Limite Diário tem de ser superior a 0.',2000);
		}
		else if(limite_semanal <= 0)
		{
			show_erro('Limite Semanal tem de ser superior a 0.',2000);
		}
		else if(limite_mensal <= 0)
		{
			show_erro('Limite Mensal tem de ser superior a 0.',2000);
		}
		else if(nome == '')
		{
			show_erro('Por Favor preencha o campo do Nome.',2000);			
		}
		else if(($('#pin').val().length<4 || $('#pin').val().length > 6)  || pin == '')
		{
			show_erro('O Pin não é válido.',2000);
		}
		else
		{
			show_erro('Formulário com erro.',2000);
		}
	}
	// return html;
}
...
function criarCartao()
{
	...
	if(telemovel != '' && matricula != '' && limite_diario > 0 && limite_semanal > 0 && limite_mensal > 0 && nome != '' && pin != null && pin != 'null' && ((pin.length > 3 && pin.length < 7) || pin == '0000'))
	{
		...
		$.ajax({
				...
				success:function(data)
				{
					...
					if(STATUS == 'SUCESS')
					{
						show_erro('Cartão criado com sucesso.',2000);
						setTimeout(function()
						{
							window.location.hash = 'iniciar-pagamento-callcenter';
						},8000);
	...
	else
	{
		...
		{
			show_erro('Por Favor preencha o campo da Matrícula.',2000);
		}
	...
	if(limite_diario > 0 && limite_semanal > 0 && limite_mensal > 0 )
	{
		...
		$.ajax({
				...
				success:function(data)
				{
					...
					if(STATUS == 'SUCESS')
					{
						show_erro('Limites alterados com sucesso.',2000);
						window.localStorage.setItem("backup_limite_diario",window.localStorage.getItem("limite_diario"));
						setTimeout(function()
						{
							window.location.hash = 'iniciar-pagamento-callcenter';
					...
					{
						show_erro('Não foi possível alterar os Limites.',2000);
					}
				},
				error:function(data)
				{
...
function requestToken()
{
	...
	$.ajax({
			...
			success:function(data)
			{
				...
				{
					show_erro('Não foi gerado token para o pagamento.',2000);					
				}
			},
			error:function(data)
			{
...
function Screen1()
{
	...
	$.ajax({
			...
			url:globalUrl+'admin_hiqi2/actions.php',
			async:false,
			data:{'type':'screen1','dp':(new Date()).getTime(),telemovel:window.localStorage.getItem("telemovel"),'token':window.localStorage.getItem("token")},
			dataType:'html',
			success:function(data)
			{
				html += data;
...
function Screen2()
{
	...
	$.ajax({
			...
			url:globalUrl+'admin_hiqi2/actions.php',
			async:false,
			data:{'type':'screen2','dp':(new Date()).getTime(),telemovel:window.localStorage.getItem("telemovel"),'token':window.localStorage.getItem("token")},
			dataType:'html',
			success:function(data)
			{
				html += data;
...
function Screen3()
{
	...
	$.ajax({
			...
			url:globalUrl+'admin_hiqi2/actions.php',
			async:false,
			data:{'type':'screen3','dp':(new Date()).getTime(),telemovel:window.localStorage.getItem("telemovel"),'token':window.localStorage.getItem("token")},
			dataType:'html',
			success:function(data)
			{
				html += data;
...
function Screen4()
{
	...
	$.ajax({
			...
			url:globalUrl+'admin_hiqi2/actions.php',
			async:false,
			data:{'type':'screen4','dp':(new Date()).getTime(),telemovel:window.localStorage.getItem("telemovel"),'token':window.localStorage.getItem("token")},
			dataType:'html',
			success:function(data)
			{
				html += data;
...
function Screen5()
{
	...
	$.ajax({
			...
			url:globalUrl+'admin_hiqi2/actions.php',
			async:false,
			data:{'type':'screen5','dp':(new Date()).getTime(),telemovel:window.localStorage.getItem("telemovel"),'token':window.localStorage.getItem("token")},
			dataType:'html',
			success:function(data)
			{
				html += data;
...
function showPinScreen()
{
	...
	html += '<div class="container container-pin" style="height:100%;background:#fff;margin-top:0px;box-shadow:1px 0px 1px 0px #ccc">';
		...
		html += '<form action="" method="" name="vform">';
			...
			html += '</div>';
		html += '</form>';
		html += '<div class="introduza-pin-label"><div class="introduza-pin-label-trad">Introduza o seu <b>PIN HiQi</b></div></div>';
	html+= '</div>';
	...
	html += 'function addNumber(element){';
		...
		html += 'else if(document.getElementById(\'mvar4\').value == "") {';
			...
			html += 'else';
			html += '{';
				html += 'setTimeout(function() {show_erro("Introduza o pin correcto.",2000);';
...
function showPinScreenPOS()
{
	...
	html += 'function addNumber(element){';
		...
		html += 'else if(document.getElementById(\'mvar6\').value == "") {';
			...
			html += 'else';
			html += '{';
				html += 'setTimeout(function() {show_erro("Introduza o token correcto.",2000);';					
					html += 'document.getElementById(\'mvar1\').value = "";';
					html += 'document.getElementById(\'mvar2\').value = "";';
					html += 'document.getElementById(\'mvar3\').value = "";';
					html += 'document.getElementById(\'mvar4\').value = "";';
...
function fazerPagamento()
{
	...
	if(cliente_mbway == 1) //INTEGRAÇÃO MBWAY, Neste momento não está em uso
	{
		$.ajax({
			type:'POST',
			timeout:10000,
			url:globalUrl+'admin_hiqi2/pgm_mbway/mbway.php',
			async:false,
			data:{
				'type':'pagarMBway',
				'dp':(new Date()).getTime(),
	...
	else //INTEGRAÇÃO FROTA. este é o que está em uso
	{
		$.ajax({
			type:'POST',
			timeout:30000,
			...
			error:function(data)
			{
				//Pode ter dado timeout mas ter desbloqueado na mesma a bomba
				show_erro('ERRO! O Serviço demorou muito tempo a responder, como a bomba pode ter ficado aberta, deve levantar e pousar a mangueira e reiniciar novamente o processo',30000);
			}
		});
	}
	return html;
